...
- El endpoint que dispara el análisis (llamada de pago al LLM) está limitado a 10 peticiones al día por usuario o IP (identificando la IP real vía Cloudflare, no vía `$request->ip()` — ver [Despliegue](#despliegue)). `CvAnalysisRateLimiter::clientIp()` descarta un valor de `CF-Connecting-IP` que no sea una IP válida, pero eso no verifica que la petición realmente haya pasado por Cloudflare: quien golpee la URL propia de Render directamente controla esa cabecera igualmente con un valor bien formado — hueco conocido, cerrarlo requeriría un paso en el propio Cloudflare (no hecho).
...
- `AnalyzeCvJob::buildSystemPrompt()`/`buildUserPrompt()` mantienen las instrucciones de la tarea (system) separadas del texto del CV/oferta subido por el usuario (etiquetado con `<cv>`/`<job_posting>` en el mensaje de usuario), en vez de concatenarlo todo en un único prompt con un delimitador fijo (`---`) que el propio contenido podía reproducir para intentar "cerrar" la sección e inyectar instrucciones nuevas. El system prompt indica explícitamente que el contenido de esas etiquetas es dato no confiable, nunca instrucciones — mitiga la inyección de prompt más obvia, no la elimina del todo: el schema estructurado sigue acotando solo el *formato* de la respuesta, no el *contenido* de cada campo.
...
- `php artisan cv-analyses:prune --days=30` (registrado a diario en `routes/console.php`) borra los análisis más antiguos que ese umbral, fila y archivo subido. Hoy en producción esa retención ya la fuerza el propio filesystem efímero del contenedor (ver [Despliegue](#despliegue)), así que este comando actúa como red de seguridad explícita, no como el mecanismo activo: si el proyecto pasa algún día a una base de datos persistente o a un plan sin spin-down, deja de depender de un efecto colateral de la infraestructura. Nota: mientras el despliegue siga siendo un único proceso `php artisan serve` sin worker/cron propio, nada dispara `schedule:run` en producción — el comando está listo y testeado (`tests/Feature/Console/PruneCvAnalysesTest.php`) pero no se ejecuta solo hasta que exista ese disparador.
