...
- GitHub Actions (`.github/workflows/tests.yml`) ejecuta la suite completa en cada push a `main`, y a continuación un chequeo de accesibilidad con axe-core (`npm run a11y`, sobre la página de subida y una página de resultado completada), un test E2E de humo (`npm run e2e`, subida → página de resultado) y Lighthouse CI (`npm run lighthouse`: rendimiento, accesibilidad, buenas prácticas y SEO, solo sobre la página de subida — ver nota en el CHANGELOG), usando el Chrome ya instalado en el runner (sin descargar Chromium propio).
