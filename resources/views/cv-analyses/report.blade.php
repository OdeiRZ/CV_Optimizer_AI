...
<body>
    ...
    <p class="subtitle">{{ $analysis->original_filename }} · generado el {{ now()->format('d/m/Y H:i') }}</p>

    <div class="score-box">
        <div class="score {{ $result['score'] >= 80 ? 'score-ok' : ($result['score'] >= 50 ? 'score-mid' : 'score-low') }}">
            {{ $result['score'] }}
        </div>
        <div class="score-label">Puntuación sobre 100</div>
        <p class="summary">{{ $result['summary'] }}</p>
    </div>

    <h2>Feedback por sección</h2>
    ...
    <p class="footer">Generado por CV Optimizer AI · análisis realizado por IA, revisa siempre el resultado con criterio propio.</p>
</body>
</html>
